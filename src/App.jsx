    ...
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
      ...
      <div style={{width:300,background:'#fff',borderRadius:22,border:'4px solid '+C.drk,overflow:'hidden',boxShadow:'0 4px 16px rgba(0,0,0,0.12)',position:'relative',display:'flex',flexDirection:'column'}}>
        ...
        <div style={{flex:1,overflowY:'auto',maxHeight:495,display:'flex',flexDirection:'column',minHeight:495}}>
          ...
          {page&&page.content({openHam:()=>setHam(true)})}
          {sos&&<SOSBar/>}
        </div>
...
function DLiveView(){
  const steps=[{ic:'✅',d:'done',l:'รับจากบ้าน',s:'09:05 น.',img:true},{ic:'✅',d:'done',l:'ถึงโรงพยาบาล',s:'10:12 น.',img:true},{ic:'⏳',d:'active',l:'กำลังรอพบแพทย์',s:'คุณทิพย์อยู่ด้วย'},{ic:'💊',d:'future',l:'รับยา',s:''},{ic:'🏠',d:'future',l:'กลับบ้าน',s:''}];
  return <div style={{padding:'8px 9px',background:C.bg}}>
    <div style={{textAlign:'center',marginBottom:6}}><div style={{fontWeight:700,color:C.pri,fontSize:13}}>{"● กำลังรอพบแพทย์"}</div><div style={{fontSize:9,color:C.mid}}>{"พ 15 พ.ค. 68 · รพ.จุฬาฯ · แม่มุก"}</div></div>
...
function GHome(){
  return <div style={{background:'#fff',padding:14,display:'flex',flexDirection:'column',gap:8}}>
    <div style={{fontSize:GF.large,fontWeight:800,textAlign:'center',color:C.drk}}>{"สวัสดีค่ะ คุณยายมุก 👋"}</div>
...
function GUpcoming(){
  ...
  return <div style={{background:'#fff',padding:14,display:'flex',flexDirection:'column',gap:10}}>
    ...
    {mode==='week'&&<><div style={{textAlign:'center'}}><div style={{fontSize:GF.large,fontWeight:800}}>{"วันพุธที่ 15 พฤษภาคม"}</div><div style={{fontSize:GF.base,color:C.mid,marginTop:4}}>{"หมอสมชาย · รพ.จุฬาฯ · 09:00 น."}</div></div><div style={{background:'#E1F5EE',borderRadius:14,padding:12,border:'2px solid #1D9E75'}}><div style={{fontSize:GF.base,color:'#085041',lineHeight:1.8}}>{"ลูกสาวจองคุณทิพย์ให้แล้วค่ะ 💛\nจะมารับบ้านเวลา 8 โมงเช้าค่ะ"}</div></div><button style={{width:'100%',height:GF.btn,borderRadius:12,background:C.suc,color:'#fff',border:'none',fontSize:GF.large,fontWeight:700,cursor:'pointer'}}>{"✅ รับทราบแล้วจ้า"}</button></>}
...
function GPINTimeline(){
  ...
  const allOk=ok0&&ok1;const ts=[{ic:'✅',d:'done',l:'รับจากบ้านแล้ว',s:'09:05 น.'},{ic:'✅',d:'done',l:'ถึงโรงพยาบาลแล้ว',s:'10:12 น.'},{ic:'⏳',d:'active',l:'กำลังรอพบหมอ',s:'คุณทิพย์อยู่ด้วยค่ะ'},{ic:'💊',d:'future',l:'รับยา',s:''},{ic:'🏠',d:'future',l:'กลับบ้าน',s:''}];
  return <div style={{background:'#fff',padding:14,display:'flex',flexDirection:'column',gap:8}}>
...
function PatientBrief(){
  ...
  return <div style={{background:'#EEEDFE',border:'1px solid #AFA9EC',borderRadius:9,padding:'8px 10px',marginBottom:6,flexShrink:0}}>
    ...
    <div style={{background:'#FFFBEB',borderRadius:6,padding:'4px 7px',marginTop:4,fontSize:8,color:'#92400E'}}>{"📋 เตรียมก่อนพบแพทย์: งดน้ำ/อาหาร 8 ชม. · X-Ray ชั้น 2 ก่อน · นำยาเดิมมาด้วย"}</div>
...
function CTJobBoard(){
  ...
  const jobs=[{type:'🤝 เพื่อนหาหมอ',hosp:'รพ.ศิริราช',date:'20 พ.ค.',pay:'฿1,680',tier:'PN',ok:true},{type:'🤝 เพื่อนหาหมอ',hosp:'รพ.จุฬา',date:'21 พ.ค.',pay:'฿1,200',tier:'CG',ok:false},{type:'🚗 รถรับส่ง',hosp:'รพ.กลาง',date:'23 พ.ค.',pay:'฿500',tier:'Driver',ok:true}];
  return <div style={{background:C.bg}}>
    ...
    <div style={{padding:'8px 9px'}}>
      ...
      <HR/>
      <ST ic="📋" ch="งานที่เปิดรับ"/>
      {jobs.map((j,i)=><Crd key={i} s={{opacity:j.ok?1:.5}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:2}}><span style={{fontSize:10,fontWeight:700}}>{j.type+" · "+j.hosp}</span><Tag ch={j.pay} col={j.ok?C.suc:C.mid}/></div>
...
function CloseHearts(){
  const[hearts,sh]=useState({s1:null,s2:null});const[rec,sr]=useState(false);
  ...
  return <div style={{padding:'8px 9px',background:C.bg}}>
    ...
    <Btn ch="🏁 ปิดงานวันนี้" col={C.suc} s={{width:'100%'}}/>
  </div>;
}
...
function RJobToday(){
  ...
  const steps=[{ic:'✅',d:'done',l:'รับจากบ้าน',t:'09:05'},{ic:'✅',d:'done',l:'ถึง รพ.',t:'10:12'},{ic:'🏥',d:'active',l:'รอพบแพทย์'},{ic:'💊',d:'next',l:'รอรับยา'},{ic:'🏠',d:'next',l:'ส่งกลับบ้าน'}];
  return <div style={{padding:'8px 9px',background:C.bg}}>
    <PatientBrief/>
    ...
    {steps.map((s,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:7,padding:'5px 8px',borderRadius:8,marginBottom:3,background:s.d==='done'?'#ECFDF5':s.d==='active'?C.pri:'#fff',border:`1px solid ${s.d==='done'?'#6EE7B7':s.d==='active'?C.pri:C.bdr}`}}>
      <span style={{fontSize:14}}>{s.ic}</span>
      <div style={{flex:1,fontSize:10,fontWeight:s.d==='active'?700:400,color:s.d==='active'?'#fff':s.d==='done'?C.suc:C.txt}}>{s.l}</div>
      {s.t&&<span style={{fontSize:8,color:s.d==='active'?'rgba(255,255,255,0.7)':C.mid}}>{s.t}</span>}
    </div>)}
...
function KBriefPIN(){
  ...
  return <div style={{padding:'8px 9px',background:C.bg}}>
    ...
    {gps&&!ok&&<Btn ch="จำลอง: คุณยายกด PIN สำเร็จ" col={C.mid} out sm s={{width:'100%',marginTop:4}} fn={()=>so(true)}/>}
...
function CDHomeMain({openHam}){
  return <div style={{background:C.bg}}>
    ...
    <div style={{padding:'8px 9px'}}>
      <Crd s={{background:'#EEEDFE',border:'1px solid #AFA9EC'}}><div style={{fontWeight:700,color:C.pri}}>{"💊 คิว A-042 · คุณแม่มุก"}</div><div style={{fontSize:9,color:C.mid}}>{"รพ.จุฬาฯ อายุรกรรม ชั้น 3"}</div></Crd>
      <div style={{display:'flex',gap:4,marginTop:4}}>
...
function OpsWebDash(){
  ...
  return <div style={{background:'#fff',borderRadius:12,border:'1px solid '+C.bdr,overflow:'hidden'}}>
    <div style={{background:C.drk,padding:'9px 13px',display:'flex',justifyContent:'space-between',flexWrap:'wrap',gap:5}}>
      ...
      <div style={{display:'flex',gap:4}}><Tag ch="⚠️ 2 SLA" col={C.wrn}/><Tag ch="🖤 3 Black" col='#374151'/><Tag ch="🟢 18 งาน" col={C.suc}/></div>
    </div>
    <div style={{background:C.bg,padding:'7px 11px',display:'flex',gap:4,flexWrap:'wrap',borderBottom:'1px solid '+C.bdr}}>
      {[{l:'งานทั้งหมด',v:'18',c:C.pri},{l:'กำลังทำ',v:'12',c:C.wrn},{l:'เสร็จ',v:'5',c:C.suc},{l:'ปัญหา',v:'1',c:C.dan},{l:'CT active',v:'14',c:C.pur},{l:'Unmatched',v:'3',c:C.org}].map((k,i)=><KPI key={i} l={k.l} v={k.v} col={k.c}/>)}
    </div>
    ...
    <div style={{padding:12}}>
      ...
      {tab==='backlog'&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        <div><ST ic="📋" ch="Unmatched"/>{[{d:'19 มี.ค.',s:'RN+Driver',z:'ลาดพร้าว',v:'฿850'},{d:'20 มี.ค.',s:'CG+Driver',z:'สุขุมวิท',v:'฿650'}].map((b,i)=><Crd key={i}><div style={{display:'flex',justifyContent:'space-between',marginBottom:2}}><span style={{fontWeight:700,fontSize:10}}>{b.d+" · "+b.s}</span><Tag ch={b.v} col={C.dan}/></div><div style={{fontSize:9,color:C.mid,marginBottom:3}}>{b.z}</div><Btn ch="จับคู่" col={C.pri} sm/></Crd>)}</div>
...
const TABS=[{id:'daughter',l:'👩 ลูกสาว'},{id:'grandma',l:'👵 คุณยาย'},{id:'rujai',l:'🤝 คุณรู้ใจ'},{id:'khabdi',l:'🚗 คุณขับดี'},{id:'dulaeh',l:'💊 คุณดูแล'},{id:'intake-chat',l:'💬 แชทจอง'},{id:'intake',l:'📝 Intake Agent'},{id:'ops_web',l:'🖥️ Ops Web'},{id:'ops_mob',l:'📱 Ops Mobile'},{id:'mgmt_web',l:'📊 Mgmt Web'},{id:'line',l:'💚 LINE'}];
...
function OpsMobView(){
  ...
  return <div style={{background:C.bg}}>
    ...
    {tab==='overview'&&<div style={{padding:'8px 9px'}}><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4,marginBottom:5}}>{[{l:'งานทั้งหมด',v:'18',c:C.pri},{l:'กำลังทำ',v:'12',c:C.wrn},{l:'เสร็จ',v:'5',c:C.suc},{l:'CT active',v:'14',c:C.pur}].map((k,i)=><div key={i} style={{background:'#fff',borderRadius:8,padding:'7px',textAlign:'center',border:'1px solid '+C.bdr}}><div style={{fontSize:16,fontWeight:800,color:k.c}}>{k.v}</div><div style={{fontSize:9,color:C.mid}}>{k.l}</div></div>)}</div><BX l="Fleet Radar" h={80} bg='#ECFDF5'/></div>}
    ...
    {tab==='backlog'&&<div style={{padding:'8px 9px'}}>{[{d:'19 มี.ค.',s:'RN+Driver',z:'ลาดพร้าว',v:'฿850'},{d:'20 มี.ค.',s:'CG+Driver',z:'สุขุมวิท',v:'฿650'}].map((b,i)=><Crd key={i}><div style={{display:'flex',justifyContent:'space-between',marginBottom:2}}><span style={{fontWeight:700,fontSize:10}}>{b.d+" · "+b.s}</span><Tag ch={b.v} col={C.dan}/></div><div style={{fontSize:9,color:C.mid,marginBottom:3}}>{b.z}</div><Btn ch="จับคู่" col={C.pri} sm/></Crd>)}</div>}
  </div>;
}
...
export default function App(){
  ...
  const rNav=[{icon:'🏠',label:'หน้าหลัก',pages:[{label:'หน้าหลัก',content:({openHam})=><RHomeMain openHam={openHam}/>}]},{icon:'🏥',label:'งานวันนี้',pages:[{label:'Brief+Steps',content:()=><RJobToday/>},{label:'ปิดงาน',content:()=><CloseHearts/>}]},{icon:'📋',label:'รับงาน',pages:[{label:'รับงาน',content:()=><CTJobBoard/>}]},{icon:'🤖',label:'น้องแคร์',pages:[{label:'Contact',content:()=><CTOpContact/>}]}];
  const kNav=[{icon:'🏠',label:'หน้าหลัก',pages:[{label:'หน้าหลัก',content:({openHam})=><KHomeMain openHam={openHam}/>}]},{icon:'🔑',label:'Brief+PIN',pages:[{label:'Brief+PIN',content:()=><KBriefPIN/>},{label:'ปิดงาน',content:()=><CloseHearts/>}]},{icon:'📋',label:'รับงาน',pages:[{label:'รับงาน',content:()=><CTJobBoard/>}]},{icon:'🤖',label:'น้องแคร์',pages:[{label:'Contact',content:()=><CTOpContact/>}]}];
  const cNav=[{icon:'🏠',label:'หน้าหลัก',pages:[{label:'หน้าหลัก',content:({openHam})=><CDHomeMain openHam={openHam}/>}]},{icon:'💊',label:'ตรวจยา',pages:[{label:'OCR',content:()=><CDashOCR/>},{label:'ปิดงาน',content:()=><CloseHearts/>}]},{icon:'📋',label:'รับงาน',pages:[{label:'รับงาน',content:()=><CTJobBoard/>}]},{icon:'🤖',label:'น้องแคร์',pages:[{label:'Contact',content:()=><CTOpContact/>}]}];
